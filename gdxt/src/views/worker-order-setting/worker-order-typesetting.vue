    ...
    <div v-show="!showWorkerDetail">
      ...
      <a-table :columns="columns" :data-source="data">
        ...
        <template slot="action" slot-scope="text,record">
          <a @click="handleRowClick(record)">设置</a>
        </template>
...
for (let i = 0; i < 6; i++) {
  ...
  const obj = Object.assign(
    ...
    {
      needsupervise: parseInt(Math.random() * 10) % 2 == 1 ? "需要" : "不需要"
    },
    { needrvisite: parseInt(Math.random() * 10) % 2 == 1 ? "需要" : "不需要" }
  );
  data.push(obj);
}
const columns = [
  ...
  {
    title: "是否要质检",
    dataIndex: "needsupervise",
    key: "needsupervise"
  },
  {
    title: "是否要回访",
    dataIndex: "needrvisite",
    key: "needrvisite"
  },
  {
    title: "发起类型",
...
export default {
  name: "TableList",
  components: {
    workerOrderDetail: () => import("./worker-order-typesetting-detail")
  },
  filters: {
