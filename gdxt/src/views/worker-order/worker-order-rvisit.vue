    ...
    <div v-show="!showWorkerDetail">
      ...
      <a-table :columns="columns" :data-source="data">
        ...
        <template slot="action" slot-scope="text,record">
          <a v-if="record.status=='未回访'" @click="handleRowClick(record)">回访</a>
          <span v-else>已回访</span>
          <!-- <a-rate v-model="rate" /> -->
        </template>
        <!-- <a slot="status" slot-scope="text">{{ parseInt(Math.random()*100)%2==1?'已回访':'未回访' }}</a> -->
      </a-table>
    </div>
...
for (let i = 0; i < 20; i++) {
  ...
  const obj = Object.assign(
    ...
    { status: parseInt(Math.random() * 100) % 2 == 1 ? "已回访" : "未回访" },
...
export default {
  name: "TableList",
  components: {
    workerOrderDetail: () => import("./worker-order-rvisit-detail")
  },
  filters: {
