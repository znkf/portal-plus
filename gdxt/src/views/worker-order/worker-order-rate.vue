    ...
    <div v-show="!showWorkerDetail">
      ...
      <a-table :columns="columns" :data-source="data">
        ...
        <template slot="action" slot-scope="text,record">
          <a-rate :value="record.rate" @change="changeRate(record,$event)" />
        </template>
...
export default {
  name: "TableList",
  components: {
    workerOrderDetail: () => import("./worker-order-approval-detail")
  },
  filters: {
  ...
  methods: {
    changeRate(row, evt) {
      console.log("rate change:", row, evt);
      const target = this.data.filter(d => row.id === d.id)[0];
      if (target) {
        target.rate = evt;
      }
    },
